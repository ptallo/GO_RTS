...
type Camera struct {
	translation objects.Point
	speed       float64
}

func NewCamera() Camera {
	return Camera{
		translation: objects.NewPoint(0, 0),
		speed:       5.0,
	}
}

func (c *Camera) Translation() objects.Point {
...
func (c *Camera) MoveCamera(p objects.Point) {
...
func (c *Camera) UpdateCameraPosition() {
	...
	if p.X() < float64(width)*0.1 {
		c.MoveCamera(objects.NewPoint(c.speed, 0))
	}
	if p.X() > float64(width)*0.9 {
		c.MoveCamera(objects.NewPoint(-c.speed, 0))
	}
	if p.Y() < float64(height)*0.1 {
		c.MoveCamera(objects.NewPoint(0, c.speed))
	}
